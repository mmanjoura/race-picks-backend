...
func getLatestSelectionsForm(selectionLink string, lastDate time.Time) ([]models.SelectionsForm, error) {
	c := colly.NewCollector()
	...
	c.OnHTML("table.FormTable__StyledTable-sc-1xr7jxa-1 tbody tr", func(e *colly.HTMLElement) {
		...
		// if the date is less than the last date, then skip
		if parsedRaceDate.Before(lastDate) {
			return
		}
